...
const SelectField: React.FC<SelectFieldType> = ({ label,name,value,defaultValue,onChange,options,error,...rest}) => {
    const optionsArray = options.map(option => ({
        name: option.name,
        value: typeof option.value === 'object' ? JSON.stringify(option.value) : option.value,
    }));
    return (
        <FormControl variant='filled' size='small' {...(error && { error: true })}>
